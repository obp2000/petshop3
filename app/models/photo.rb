...
class Photo < ItemAttribute
  ...
  def must_have_photo
    errors.add :base, "Не выбрана #{class_name_rus} для загрузки" unless photo_before_type_cast     
  end
...
# links
  def link_to_show( page, comment = "" ); page.link_to page.image_tag( photo.thumb.url ) + comment, photo_url end
