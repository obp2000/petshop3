...
describe "forum_posts/_forum_post" do
  ...
  before(:each) do
    ...
    view.stub( :link_to_show ).with( @forum_post ).and_return( link_to_remote @forum_post.subject,
            :url => forum_post_path( @forum_post ), :method => :get )
    view.stub( :link_to_delete ).with( @forum_post ).and_return( link_to_remote "Test",
  ...
  it "renders forum post" do
    ...
    rendered.should have_text( regexp_for_remote_delete( forum_post_path( @forum_post ) ) )       
  end
